...
// pipeline), LEAVING meeting_booked forward auto-logs the 'meeting' activity
// the Dashboard funnel counts (meeting_booked means "agreed to meet", so the
// log fires on the way OUT, once the meeting has actually happened), and
// neither side effect may ever fire twice or break the stage change itself.
...
describe('moveLead — meeting auto-log', () => {
  it('auto-logs a meeting activity when leaving meeting_booked forward into warm_active', async () => {
  ...
  it('also logs when meeting_booked jumps straight to client', async () => {
    h.db = db({ priorStage: 'meeting_booked' })
    await moveLead('lead-1', 'client', 'p1')
    expect(activityTypes()).toContain('meeting')
  })

  it('does not log on ENTERING meeting_booked — the meeting has not happened yet', async () => {
    h.db = db({ priorStage: 'responded' })
    await moveLead('lead-1', 'meeting_booked', 'p1')
    expect(activityTypes()).not.toContain('meeting')
  })

  it('does not re-log when staying at meeting_booked', async () => {
    h.db = db({ priorStage: 'meeting_booked' })
    await moveLead('lead-1', 'meeting_booked', 'p1')
    expect(activityTypes()).not.toContain('meeting')
  })

  it('does not log a meeting for any other transition', async () => {
    h.db = db({ priorStage: 'outreach' })
    await moveLead('lead-1', 'responded', 'p1')
    expect(activityTypes()).not.toContain('meeting')
  })
...
describe('moveLead — the stage change itself', () => {
  it('writes the new stage and logs an audit note', async () => {
    h.db = db({ priorStage: 'outreach' })
    const result = await moveLead('lead-1', 'warm_active', 'p1')

    expect(h.db.opsFor('crm_leads', 'update')[0].payload).toEqual({ stage: 'warm_active' })
    expect(result.stage).toBe('warm_active')
